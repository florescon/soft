<div class="relative flex flex-col min-w-0 mt-6 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border" id="basic-info">
    <div class="pt-6 pl-6 pr-6 mb-0 rounded-t-2xl">
        <h5 class="dark:text-white">{{ __('Edit Ingredient') }}</h5>
        <p class="dark:text-white">{{ __('Edit') }} {{ $ingredient->name }}</p>
            ...
            <div class="flex flex-wrap -mx-3">
                <div class="w-6/12 max-w-full px-3 flex-0">
                    ...
                    <div class="relative flex flex-wrap items-stretch w-full rounded-lg">
                        <input wire:model.blur="name" value="{{ $ingredient->name }}" type="text" placeholder="{{ __('Enter name') }}" class="focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" required />
                    </div>
