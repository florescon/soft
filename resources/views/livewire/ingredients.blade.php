<div class="w-full max-w-full px-3 flex-0">
    <div class="relative flex flex-col min-w-0 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="pt-6 pr-6 pl-6 mb-0 rounded-t-2xl">
            <h5 class="dark:text-white">{{ __('Ingredient Management') }}</h5>
            <p class="dark:text-white"> {{ __('Here you can manage ingredients.') }} </p>
        </div>
        <div class="my-auto ml-auto pr-6">
            <a href="{{ route('ingredient-create') }}"><button type="button" class="inline-block px-8 py-2 m-0 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-gradient-fuchsia shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85">+&nbsp;
            {{ __('Add Ingredient') }}</button></a>
        </div>
                    ...
                    @forelse($ingredients as $ingredient)
                        <tr wire:key="row-{{ $ingredient->id }}">
                            ...
                            <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent dark:border-white/40">
                                <p class="mb-0 font-semibold leading-tight text-size-xs text-slate-400 dark:text-white/80">
                                {{ $ingredient->created_at_for_humans }}</p>
                            </td>
                            <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent dark:border-white/40">
                                <p class="mb-0 font-semibold leading-tight text-base">
                                    <a rel="tooltip" href="{{ route('ingredient-edit', $ingredient->id) }}">
                                    ...
                                    <button type="button" onclick="confirm('Are you sure you want to delete this ingredient?') || event.stopImmediatePropagation()" wire:click="destroy({{ $ingredient->id }})">
