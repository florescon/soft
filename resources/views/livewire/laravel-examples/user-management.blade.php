    ...
    <div class="flex-none w-full max-w-full px-3">
        <div
            ...
            <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6>{{ __('All users') }}</h6>
                <p>{{ __('Here you can manage users.') }}</p>
            </div>
                ...
                <div class="p-0 overflow-x-auto">
                    <div class="mb-4 flex justify-between pl-6 pr-6 pt-4">
                        ...
                        <div class="relative flex flex-wrap items-stretch transition-all rounded-lg ease-soft" style="max-height: 40.3693px">
                            ...
                            <input wire:model.live="search" maxlength="40" type="text" class="pl-9 text-size-sm focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 ease-soft w-1/100 leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" placeholder="{{ __('Search') }}..." />
                        </div>
                    </div>
                        ...
                        <tbody>
                            @forelse($users as $user)
                                <tr>
                                    ...
                                    <td
                                        class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                        <p class="mb-0 font-semibold leading-tight text-size-xs">{{ $user->created_at_for_humans }}</p>
                                    </td>

                                    <td
                                        class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                        <p class="mb-0 font-semibold leading-tight text-base">
                                            <a rel="tooltip" href="{{ route('user-edit', $user->id) }}">
                                            ...
                                            <button type="button" onclick="confirm('Are you sure you want to delete this user?') || event.stopImmediatePropagation()" wire:click="destroy({{ $user->id }})">
